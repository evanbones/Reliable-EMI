...
public class StackGroupManager {
    ...
    public static ResourceLocation getTagGroupId(TagKey<?> tagKey) {
        ...
        return new ResourceLocation(tagKey.location().getNamespace(), prefix + tagKey.location().getPath());
    }
                    ...
                    try (var reader = Files.newBufferedReader(path)) {
                        ...
                        if (json.has("id")) {
                            ...
                            if (registry.equals("minecraft:item")) {
                                ...
                                idString = new ResourceLocation(tagLoc.getNamespace(), registry.replace(':', '_') + "_" + tagLoc.getPath()).toString();
                            }
                        } else {
                            idString = null;
                        }
                        if (idString != null) {
                            ResourceLocation resId = new ResourceLocation(idString);
        ...
        for (EmiStack emiStack : source) {
            ...
            if (variants == null) {
                ...
                for (var v : idVariants) {
                    if (v.realStack.isEqual(emiStack, Comparison.compareNbt())) variants.add(v);
                }
    ...
    public static List<EmiIngredient> buildGroupedIngredients(List<? extends EmiIngredient> source, SidebarType type) {
        ...
        for (EmiIngredient ing : source) {
            ...
            EmiStack primary = ing instanceof EmiStack es ? es : (!ing.getEmiStacks().isEmpty() ? ing.getEmiStacks().get(0) : null);
            if (primary != null) {
                ...
                if (variants == null) {
                    ...
                    if (idVariants != null) {
                        variants = new ArrayList<>();
                        for (var v : idVariants) {
                            if (v.realStack.isEqual(primary, Comparison.compareNbt())) variants.add(v);
                        }
                    }
                }
                if (variants != null) {
    ...
    private static void registerMatch(StackGroup group, EmiStack stack, Map<StackGroup, EmiGroupStack> groupStacksMap) {
        ...
        if (added && group.isEnabled) {
            ...
            for (EmiStack gs : groupedEmiStacks) {
                if (gs.isEqual(stack, Comparison.compareNbt())) {
                    alreadyGrouped = true;
                    break;
                }
            }
